...
from fieldsim.utils.exceptions import WrongCoordsFormatError
from fieldsim.utils.exceptions import WrongCoordsLengthError


class SkySource:
    def __init__(self, coords, m_min=1, m_max=350):
        """
        Class that initializates a `SkySource` instance, generating its mass with a given IMF and determining its
        luminosity, given a mass-luminosity relation.

        At initialization, the class saves the coordinates of the source (relative to the field) and the mass limits
        to be used for a powerlaw Initial Mass Function.
        ...
        >>> # Generates a source in the same location, but with default mass boundaries and location defined with a
        >>> # numpy array.
    ...
    def initialize(self, e_imf=2.4, e_lm=3, cst_lm=1, rng=None):
        ...
        It uses a random number generator from numpy to generate a random number in the [0, 1) interval. This value
        is, then, passed to the `self.random_cimf` method, to extract a mass in the [`self.m_min`, `self.m_max`)
        interval. Luminosity is, then, derived with a powerlaw defined in the `self.lm_relation` method. A
        magnitude-like quantity, defined in the `self.l2mag` method.

        Parameters
        ----------
        e_imf: `number`
            Exponent of the IMF powerlaw, to be passed to the `self.random_cimf` method. Default is `2.4`.
        e_lm: `number`
            Exponent of the mass-luminosity relation powerlaw, to be passed to the `self.lm_relation` method.
            Default is `3`.
        cst_lm: `number`
            Factor in front of the mass-luminosity relation powerlaw, to be passed to the `self.lm_relation` method.
            Default is `1`.
        rng: `numpy.random.Generator` or `None`
            If of type `numpy.random.Generator`, it uses the given pseudo-random number generator. If `None`,
            the `numpy.random.default_rng()` generator is used with a casual seed. Default is `None`. (Modified on
            May 5th 2021)
        ...
        ------
        `TypeError`:
            If `rng` is not an `np.random.Generator` or `None`. (Modified on May 5th 2021)

        Examples
        --------
        >>> # Initializes the mass of the source with the Salpeter IMF. The luminosity powerlaw is set for a Main
        >>> # Sequence star, but the factor of the L-M relation is 100, to obtain higher values for the luminosity.
        >>> src.initialize(e_imf=2.35, e_lm=3, cst_lm=100)

        See Also
        --------
        `SkySource.random_cimf`, `SkySource.lm_relation`, `SkySource.l2mag` and `numpy.random.default_rng` or
        `numpy.random.Generator`. (Modified on May 5th 2021)
        """
        if not isinstance(rng, np.random.Generator) and rng is not None:
            raise TypeError("`rng` argument must be either a `numpy.random.Generator` or `None`.")
    ...
    def random_cimf(self, cimf, e=2.4):
        """
        Method that generates a mass from a powerlaw IMF, given a randomly generated value in the interval [0, 1].

        It uses the cdf of a powerlaw defined on the interval [`m_min`, `m_max`], where the limits are given at
        instance creation. The mass corresponding to a given `cimf` is given by:
        ...
        >>> # for the default mass limits.
        >>> src.random_cimf(0.3, e=2.4)
        1.2900536896433321

        >>> # Mass of a star following an IMF defined by the powerlaw M^-2.4, in case the random variable is 0.6, for
        >>> # for the default mass limits.
        >>> src.random_cimf(0.6, e=2.4)
        1.9236020460960432

        >>> # Mass of a star following an IMF defined by the powerlaw M^-2.4, in case the random variable is 0.95, for
        >>> # for the default mass limits.
        ...
        # A max value was set because, even if unlikely, irrealistically massive stars have been extracted.
    ...
    def lm_relation(self, e=3, cst=1):
        ...
        if self.mass is None:
            raise AttributeError("Star must be generated, before luminosity can be defined.")
    ...
    def l2mag(self):
        ...
        Examples
        --------
        >>> # If the star has a luminosity of 10 arb. lum. units, this method returns:
        >>> src.l2mag()
        -2.5
        """
        if self.luminosity is None:
            raise AttributeError("Star must be generated and have a luminosity defined, before magnitude can be "
                                 "defined.")
    ...
    def m_min(self):
        """
        Property of a `SkySource` type object. It is the lower mass limit for the IMF.
        """
        return self.__m_min
    ...
    def m_max(self):
        """
        Property of a `SkySource` type object. It is the higher mass limit for the IMF.
        """
        return self.__m_max
