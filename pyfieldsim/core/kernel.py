class Kernel:
    def __init__(self, name, width_x, width_y, size):
        """
        Class that initializates a generic kernel for a Point Spread
        Function (psf).

        When initialized, it stores a name for the kernel and typical widths
        on the x and y-axis (`width_x` and `width_y`) and the size of the
        kernel as the multiplying factor with respect to the typical widths.
        ...
        size:
            The multiplying factor to obtain the shape of the matrix
            defining the kernel.

        Examples
        --------
        >>> # Example on the `size` argument. In this case the kernel has
        >>> # not central simmetry and the matrix
        >>> # representing it will be an
        >>> # `int(2.5 * width_x)` x `int(2.5 * width_y)` matrix.
        >>> generic_kernel = Kernel(
        >>>     name='generic', width_x=10, width_y=8, size=2.5
        >>> )
