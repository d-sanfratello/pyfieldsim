...
class GaussKernel(Kernel):
    def __init__(self, sigma, size=2.5):
        ...
        It defines a square kernel array with axis of len `int(size * sigma)`,
        ...
        ----------
        sigma:
            The standard deviation of a 2D gaussian distribution with central
            symmetry.
        size:
            The multiplier to obtain the size of the kernel. In this case it
            corresponds to `size` sigmas. Default is `2.5`.
        """
...
class AsymmGaussKernel(Kernel):
    def __init__(self, sigma_x, sigma_y, size=2.5):
        ...
        It defines a rectangular kernel array of shape `(int(size * sigma_x),
        int(size * sigma_y)`, where `sigma_x` and `sigma_y` are the standard
        deviations of the distributions along the two axis. The kernel is
        normalized.

        Parameters
        ----------
        sigma_x:
            The standard deviation of a 2D gaussian distribution with axis
        ...
        size:
            ...
            corresponds to `size` sigmas. Default is `2.5`.
        """
        super(AsymmGaussKernel, self).__init__(
            'asymm_gauss', sigma_x, sigma_y, size)
    ...
    def _generate_kernel(self):
        ...
        return np.exp(-0.5 * (xx ** 2 + yy ** 2)) / (2 * np.pi * self.width_x *
                                                     self.width_y)
