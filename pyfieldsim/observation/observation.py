...
class Observation:
    ...
    def __init__(self, field, ext='fits'):
        """
        Class that initializates an `Observation` instance, to simulate an
        exposure through a telescope and its following analysis.

        At initialization the class takes a `field` argument, either a
        `field.Field` object, a string to the path or a `Path` object and,
        in the last two cases, a extension `ext` being either `"fits"` or
        `"text"`. If `field` is a `Field` object, argument `ext` is ignored.

        Parameters
        ----------
        field: `field.Field`, string or `path.Path`
            Either a `Field` type object or the path to a .fits file or .txt
            file.
        ext: string either `"fits"` or `"txt"`
            If `field` is a path, the extension of the file to open. Default is
            `"fits"`.
    ...
    def __extract_field(self, field, ext):
        """
        Method that called at `observation.Observation` initialization and
        whenever the saved image needs to be updated. This method extracts an
        exposure from `field` for a set type of data file defined in `ext`.

        Returns
        -------
        shot: `numpy.ndarray`
            Returns the property `shot` of an instance of `field.Field`, if
            `field` is such instance, or the matrix representing the exposure as
            extracted from the .txt or the .fits file.
        """
        if isinstance(field, Field):
            ...
            if ext == 'fits':
                ...
                with open(field, 'r') as imfile:
                    ...
                    self.status = _status_values[
                        (header_list[self.__lst_header['status']])]
                    self.datatype = _datatype_values[
                        (header_list[self.__lst_header['datatype']])]

                    image = np.array([[float(px) for px in line.split()]
                                      for line in lines[1:]])
            ...
            raise IOError("Unknown field passed as argument. Must either be a "
                          "`Field` object or a path.")

    def update_image(self):
        """
        Method to update the image stored inside an `Observation` instance.

        In the case of an image generated from a `field.Field` object, any step
        in the simulation updates the `Field.shot` property. This method allows
        for the stored image to be update accordingly.

        Notice that this method works even if the image is extracted from a
        file, but it won't change the stored image unless the file was actually
        different, since it would simply re-read its content.
    ...
    def count_single_stars(self):
        ...
        """
        if self.status not in [ImageStatus().SINGLESTARS,
                               ImageStatus().PH_NOISE]:
        ...
        if self.datatype == DataType().MAGNITUDE:
            # In this implementation, the magnitude-like quantity cannot be
            # greater than 0. So it is natural to look for the minima.
            ...
            while image_brighter < 0:
                ...
                image_copy[coords] = 0
                image_brighter = image_copy.min()
        elif self.datatype == DataType().LUMINOSITY or \
                self.datatype == DataType().MASS:
